...
public class EmployeeController {
    ...
    public ResponseEntity<CollectionModel<EntityModel<UiEmployee>>> getAll() {
        return ResponseEntity.ok(CollectionModel.of(employeeBF.getAll(employeeToUiMapper).stream()
                                                            .map(employee -> EntityModel.of(employee).add(linkTo(
                                                                    methodOn(EmployeeController.class)
                                                                            .getById(employee.getEmployeeId())).withRel(
                                                                    GET_EMPLOYEE.toString()))).collect(Collectors.toList()))
                                         .add(linkTo(methodOn(EmployeeController.class).createEmployee(null))
                                                      .withRel(CREATE_EMPLOYEE.toString()))
    ...
    public ResponseEntity<EntityModel<UiEmployee>> getById(@PathVariable final Integer id) {
        return ResponseEntity.ok(EntityModel.of(employeeBF.getById(id, employeeToUiMapper))
                                         .add(linkTo(methodOn(EmployeeController.class).getAll())
                                                      .withRel(GET_ALL_EMPLOYEES.toString()))
